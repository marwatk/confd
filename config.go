...
func init() {
	...
	flag.Var(&config.YAMLFile, "file", "the YAML file to watch for changes (only used with -backend=file or -backend=clconf)")
	flag.Var(&config.YAMLBase64, "file-base64", "base64 encoded YAML (only used with -backend=clconf)")
